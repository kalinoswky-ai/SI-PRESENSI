...
export default async function AdminLeaveTypePage({
  ...
  if (!VALID_TYPES.includes(params.type as LeaveType)) notFound();
  ...
  return (
    <div className="space-y-4">
      <div>
        <Link href="/admin/leave" className="text-xs font-medium text-brand-600 hover:underline">
          ← Semua folder Time Off
        </Link>
      ...
      <div className="flex gap-1 overflow-x-auto">
        {TABS.map((t) => (
          <a
            key={t.key}
            href={`/admin/leave/${type}?status=${t.key}`}
      ...
      {!leaveError && records.length === 0 && (
        <p className="text-sm text-slate-500">Tidak ada pengajuan {typeLabel.toLowerCase()} pada kategori ini.</p>
      )}
