...
export async function POST(request: NextRequest) {
  ...
  // 3d. Tentukan mode kerja. WFH (bebas radius kantor) berlaku bila:
  //       (a) pegawai punya pengecualian apel yang disetujui & berlaku hari ini — boleh absen
  //           dari rumah masing-masing, tidak wajib ke kantor; ATAU
  //       (b) hari Jumat & kebijakan Jumat hybrid aktif (sesuai pilihan pegawai).
  //     Di luar itu selalu WFO (wajib geofencing) berapa pun yang dikirim klien.
  const hybridFriday = Boolean(office.friday_hybrid) && isFridayWita(serverTime);
  let workMode: WorkMode = "wfo";
  if (type === "in") {
    if (apelExempted) {
      workMode = "wfh";
    } else if (hybridFriday) {
  ...
  // Radius kantor HANYA diwajibkan untuk absen MASUK mode WFO (termasuk yg belum/tidak
  // dikecualikan dari apel). Pegawai yang dikecualikan dari apel sudah bermode WFH (lihat
  // 3d di atas) sehingga tidak lagi kena wajib radius kantor di sini. Absen PULANG tidak
  // pernah ditolak karena lokasi — pegawai yang audit/tugas lapangan hingga lewat jam kantor
  // tetap bisa absen pulang; titik koordinat sebenarnya tetap direkam untuk jejak audit.
  if (beforeWorkEnd) {
    status = "rejected";
